...
def apply_discount(price: float, discount_tier: str) -> float:
    ...
    if price is None:
        return "Error: price is required. Call get_product_price first to obtain the price."
...
def run_agent(question: str):
    ...
    # Track which tools have been called to enforce correct ordering
    called_tools = set()
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        # Guardrail: force model to call get_product_price before apply_discount
        if tool_name == "apply_discount" and "get_product_price" not in called_tools:
            observation = (
                "Error: You must call get_product_price first to get the real price. "
                "Do NOT make up a price. Call get_product_price now."
            )
            print(f"  [Tool Blocked] {observation}")
        else:
